...
from memora_repro.retrieval.memora import MemoraSemanticRetriever, format_memory_context
...
def main() -> None:
    ...
    out_file = Path(config["paths"]["results_dir"]) / "memora_s_predictions.jsonl"
    ...
    retriever = MemoraSemanticRetriever(
        client=client,
    ...
    for qa in tqdm(qas, desc="MEMORA(S)"):
